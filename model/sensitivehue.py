...
from backbone.attention import MultiHeadAttn
from backbone.revin import RevIN
...
class SensitiveHUE(nn.Module):
    ...
    def __init__(self, step_num_in: int, f_in: int, dim_model: int, head_num: int, dim_hidden_fc: int,
                 encode_layer_num: int, dropout=0.1, alpha=1.0, beta=1.0):
        super().__init__()
        self.f_in = f_in
        self.eps = 1e-5
        self.alpha = alpha
    ...
    def forward(self, x: torch.Tensor, mask=None, return_feature=False):
